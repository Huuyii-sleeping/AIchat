<template>
  <div class="flex items-center py-4">
    <div class="w-[100px]">{{ t("settings.providers.defaultModel") }}：</div>
    <n-select
  ...
  <n-collapse>
    <n-collapse-item
      v-for="(provider, index) in providersStore.allProviders"
      :key="provider.name"
      :title="provider.title ?? provider.name"
    >
      <template #header-extra>
        <n-switch
          :value="providersStore.allProviders[index].visible"
          :disabled="
            !providersStore.allProviders[index].openAISetting.apiKey ||
            !providersStore.allProviders[index].openAISetting.baseURL
          "
          @update:value="
            (v) => providersStore.updateProvider(provider.id, { visible: v })
          "
          @click.stop
        />
      </template>
      ...
      <n-dynamic-tags
        :value="providersStore.allProviders[index].models ?? []"
        @update:value="
          (v: any) => providersStore.updateProvider(provider.id, { models: v })
        "
      />
    </n-collapse-item>
  </n-collapse>
...
function handleApiKeyUpdate(id: number, apiKey: string) {
  const baseURL =
    providersStore.allProviders.find((item) => item.id === id)?.openAISetting
      ?.baseURL ?? "";

  const update: Partial<Provider> = {
    openAISetting: stringifyOpenAISetting({ baseURL, apiKey }),
  };
